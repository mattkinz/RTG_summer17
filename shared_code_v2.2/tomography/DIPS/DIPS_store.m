function [Uall,out,Uset] = DIPS_store(opts,stack,init)
...
if slices~=1
    error('dirt_store only runs on one slice');
end

Uall = zeros(n,n,opts.dips_iter+1);
Uset = zeros(n*n,opts.dips_iter);
...
for k = 1:numchunks
    ...
    for i = 1:opts.dips_iter
       ...
       %{     
    	...
    	Uchunk=TVAL3D(A,b3,n,n,chunksizes(k),opts);
...
if opts.bdry_dirt_iter + opts.region_dirt_iter>0
    ...
    [U,out_dirt,Uset2] = DIRT_store(opts,bb,U);
    out.dirt = out_dirt;
    Uall = cat(3,Uall,U);
    Uset = cat(3,Uset,Uset2);
else
    %Smooth once more and segment
   % T = computegaussian(opts.rx,opts.rz,2*opts.sigmax,2*opts.sigmaz,slices,opts.chunksize);
    Uall(:,:,end) = threshold(imfilter(U,T,'replicate'),opts.thresh,opts.grays);
