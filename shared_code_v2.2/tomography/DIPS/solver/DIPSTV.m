function [U, out] = DIPSTV(A,b,b3,col_fixed,n,opts)
...
[b,scl] = Scaleb(b);
b3 = b3*scl;  % update fixed values

% check for min/max constraint values
if opts.max_c
    max_v = opts.max_v*scl;
end
if opts.min_c 
    min_v = opts.min_v*scl;
end
...
[U,mu,beta,muf,betaf,muDbeta,sigma,delta,gL,ind,out] ...
    = get_DIPSTV(p,q,r,Atb,scl,opts,k,b,wrap_shrink);    % U: p*q
...
for ii = 1:opts.outer_iter
    ...
    for jj = 1:opts.inner_iter
        ...
        U(col_fixed) = b3;  % reproject fixed pixels
