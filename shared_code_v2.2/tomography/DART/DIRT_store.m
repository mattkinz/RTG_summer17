function [Uall,out,Uset] = DIRT_store(opt,bb,init)
...
for k = 1:numchunks
    ...
    if ~given
        ...
        init(:,:,v1(k,1):v1(k,2)) = sirtden_nodisplay(bchunk,W,n,30,opt.grays(1));
    end
    ...
    c_tol=opt.t_tol;
    Uall = zeros(n,n,opt.total_iter+1);
    Uset = zeros(n^2,opt.total_iter);
    ...
    for i = 1:opt.total_iter
        ...
        Usmooth = imfilter(Uchunk,T,'replicate');
        
        
        [Uchunk,Ucheck,set,fixed,update_type,type,convergence] = ...
            DIRT_thresh(Usmooth,opt,i,c_tol,Ucheck);
        Uall(:,:,i+1)=Uchunk;
        Uset(set{1},i) = 1;
        ...
        Usmooth = reshape(Usmooth,n^2,chunksizes(k));
        for j = 1:chunksizes(k)
           Usmooth(fixed{j},j) = Uchunk(fixed{j},j); 
        end
        ...
        Uchunk = DIRT_update(W,Wn,bchunk,Usmooth,set,fixed,update_type,opt,c,n);
...
%}
Uset = reshape(Uset,n,n,size(Uset,2));
