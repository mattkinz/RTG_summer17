function [D,Dt] = FD3D_multiscale_complex(k,levels,p,q,r,theta)
...
for ii = 1:levels
    vx = [0,1/l(ii)*((exp(-1i*2*pi*(1:q-1)*l(ii)/q)-1).^(k+1))./(exp(-1i*2*pi*(1:q-1)/q)-1)];
    vy = [0,1/l(ii)*((exp(-1i*2*pi*(1:p-1)*l(ii)/p)-1).^(k+1))./(exp(-1i*2*pi*(1:p-1)/p)-1)];
    vz = [0,1/l(ii)*((exp(-1i*2*pi*(1:r-1)*l(ii)/r)-1).^(k+1))./(exp(-1i*2*pi*(1:r-1)/r)-1)];
...
D = @(U)D_Forward(U,VX,VY,VZ,p,q,r,levels,theta,k);
Dt = @(Uc)D_Adjoint(Uc,VX,VY,VZ,p,q,r,levels,theta,k);


% high order finite differences
function [dU] = D_Forward(U,VX,VY,VZ,p,q,r,levels,theta,k)
...
%transpose FD
function dtxy = D_Adjoint(dU,VX,VY,VZ,p,q,r,levels,theta,k)
    ...
    dtxy = 2^(1-k)/levels*(X(:) + Y(:) + Z(:)).*conj(theta(:));
