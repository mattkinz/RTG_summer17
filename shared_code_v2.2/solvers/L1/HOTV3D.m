...
%   lam4: (sigma,Du-w)
%   lam5: (delta,Au-b)
%when frequencies are not gridded---------------------------------------
if numel(n)<3
    n(end+1:3) = 1;
elseif numel(n)>3
...
if round(k)~=k || opts.levels>1
    wrap_shrink = true; % indices aren't set up for these cases, ¯\_(ツ)_/¯ so just do shrinkage everywhere
end
...
if ~isa(A,'function_handle')
    A = @(u,mode) f_handleA_matt_2(A,u,mode);
end
