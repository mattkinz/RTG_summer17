...
%	pr f.snr_wrong
	...
	slim = [0 1.4];
	im(2, abs(zlj), '$|$surface coil images $z_{lj}|$'), cbar
	im(3, abs(smap), '|smap|', slim), cbar
	drawnow
end
...
if ~isvar('shat1'), printm 'ratio'
	...
	im(8, 'hsv', angle(shat1-smap), '$\angle$ err', alim), cbar(atick)
	drawnow
prompt
end
...
if ~isvar('shat2'), printm 'qpls via cholesky'
	...
	im(9, 'hsv', angle(shat2-smap), '$\angle$ err', alim), cbar(atick)
	drawnow
prompt
end
...
if 1
	...
	im(2, 'hsv', angle(zlj), '$\angle$ zlj', alim), cbar(atick)
	im(3, 'hsv', angle(smap), '$\angle$ smap', alim), cbar(atick)
	im(4, 'hsv', angle(shat1), '$\angle$ ratio', alim), cbar(atick)
	im(7, 'hsv', afun(shat1, smap), '$\angle$ err', alim), cbar(atick)
	tfun(shat1, smap)
	im(5, 'hsv', angle(shat2), '$\angle$ qpls:chol', alim), cbar(atick)
	im(8, 'hsv', afun(shat2, smap), '$\angle$ err', alim), cbar(atick)
	tfun(shat2, smap)
	im(6, 'hsv', angle(shat3), '$\angle$ qpls:iter', alim), cbar(atick)
	im(9, 'hsv', afun(shat3, smap), '$\angle$ err', alim), cbar(atick)
	tfun(shat3, smap)
